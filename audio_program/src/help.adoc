...
Note constants are available in scientific pitch notation using MIDI note 60 = C4. Lowercase notes push frequencies (`c4` = 261.625565, `a4` = 440), uppercase notes push MIDI numbers (`C4` = 60, `A4` = 69). Sharps and flats are supported (`c#4`, `Db4`).
...
[horizontal]
cycle, cy:: (cps) -> wrapped `0..1` cycle phase, advanced by cycles-per-second input; negative CPS runs backwards
pat:<PATTERN>:: (phase) -> read a flat comma-separated numeric pattern using wrapped `0..1` phase, e.g. `pat:60,64,67,72`
gate:<PATTERN>:: (phase) -> held gate from a flat dense pattern where `x`/`X` is active and `.` is inactive, e.g. `gate:x..x`
trig:<PATTERN>:: (phase) -> one-sample trigger on entering an active cell of a flat dense pattern, e.g. `trig:x..x`
